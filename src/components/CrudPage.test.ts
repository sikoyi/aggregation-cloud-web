...
describe('账号和设备管理批量操作栏', () => {
  it('仅账号和设备管理常驻，其他资源仍按选中状态显示，并保留权限过滤', () => {
    expect(source).toContain("computed(() => ['accounts', 'accountIdentities'].includes(props.config.key))")
    expect(source).toContain("computed(() => isAccountResource.value || props.config.key === 'slots')")
  ...
  it('设备选择数量使用台设备作为单位', () => {
    expect(toolbar).toContain("config.key === 'slots' ? '台设备' : '条数据'")
