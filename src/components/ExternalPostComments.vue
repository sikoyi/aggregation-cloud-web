...
function commentRows(value: unknown) {
  ...
  return rows.map((item, index) => {
    ...
    return {
      ...
      content: metadata.media_only && media.length ? '' : String(item.content || '无文字内容'), time: item.commented_at,
      ...
      replyTo: parentId ? names.get(parentId) || '未采集的评论' : '',
    }
  })
}
...
<template>
  <section class="external-comments" aria-label="已采集评论">
    ...
    <ol v-if="rows.length" class="external-comments__list">
      <li v-for="comment in visibleRows" :key="comment.key" class="external-comment" :class="{ 'is-reply': comment.replyTo }">
        ...
        <div class="external-comment__body">
          ...
          <span v-if="comment.replyTo" class="external-comment__reply"><CornerDownRight :size="13" />回复 {{ comment.replyTo }}</span>
...
.external-comment__body p { white-space: pre-wrap; overflow-wrap: anywhere; margin: 8px 0 10px; line-height: 1.7; font-size: 14px; color: #334e68; }
...
.external-comment__reply { margin-top: 7px; font-size: 12px; color: #64809a; overflow-wrap: anywhere; }
