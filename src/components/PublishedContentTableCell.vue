...
.published-task-identity > strong { display: block; overflow: hidden; color: var(--app-text, #243b53); font-size: 13px; text-align: center; text-overflow: ellipsis; white-space: nowrap; }
...
.published-content__title > strong { overflow: hidden; color: var(--app-text, #243b53); font-size: 13px; text-overflow: ellipsis; white-space: nowrap; }
.published-content__media { display: inline-flex; flex: 0 0 auto; align-items: center; gap: 3px; color: var(--app-text-muted, #527a98); font-size: 11px; }
.published-content__media svg { width: 13px; height: 13px; }
.published-content p { display: -webkit-box; overflow: hidden; margin: 0; color: var(--app-text-muted, #6b7f93); font-size: 12px; line-height: 18px; overflow-wrap: anywhere; word-break: break-word; -webkit-box-orient: vertical; -webkit-line-clamp: 2; }
...
.published-publisher__row > svg { width: 14px; height: 14px; flex: 0 0 14px; color: var(--app-blue, #39749a); }
...
.published-publisher__row strong { color: var(--app-text, #334e68); font-size: 12px; }
.published-publisher__row small { color: var(--app-text-muted, #8a9aab); font-size: 10px; }
.published-publisher__row--secondary > svg { color: var(--app-text-muted, #7b8c9d); }
...
.published-link small { overflow: hidden; max-width: 100%; color: var(--app-text-muted, #8293a5); font-family: ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, monospace; font-size: 10px; text-overflow: ellipsis; white-space: nowrap; }
...
.published-metrics > span {
  ...
  padding: 6px 8px;
  border-radius: 5px;
  color: var(--app-text-muted, #60778d);
...
.published-metrics strong { color: var(--app-text, #263f55); font-size: 12px; }
...
.published-monitor-state strong { color: var(--app-text, #526578); font-size: 12px; }
.published-monitor-state small { color: var(--app-text-muted, #8a9aab); font-size: 10px; }
.published-monitor-state.is-abnormal { color: var(--app-amber, #c47b2a); background: var(--app-surface-muted, #fff8ed); }
.published-monitor-state.is-abnormal strong { color: var(--app-amber, #9a5e1d); }
.published-monitor-state.is-abnormal small { color: var(--app-amber, #b27a3b); }
.published-time { display: flex; align-items: center; justify-content: center; gap: 7px; color: var(--app-text, #40566c); white-space: nowrap; }
.published-time svg { width: 14px; height: 14px; color: var(--app-text-muted, #527a98); }
...
.published-task-result__progress > span { color: var(--app-text-muted, #526a7f); font-size: 11px; }
...
.published-task-result__counts .is-success { color: var(--app-green, #3e7c55); }
.published-task-result__counts .is-failed { color: var(--app-red, #c04b4b); }
.published-task-result__counts .is-canceled { color: var(--app-text-muted, #7c8794); }
.published-task-output { display: flex; flex-direction: column; gap: 8px; }
.published-task-output > span { color: var(--app-text-muted, #527a98); }
.published-task-output small { min-width: 54px; color: var(--app-text-muted, #73879a); font-size: 11px; }
.published-task-output strong { color: var(--app-text, #263f55); }
.published-task-output .has-missing,
.published-task-output .has-missing small,
.published-task-output .has-missing strong { color: var(--app-red, #c04b4b); }
</style>
