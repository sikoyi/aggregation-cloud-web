...
export const resources: Record<string, ResourceConfig> = {
  ...
  taskTemplates: {
    ...
    createFields: [
      ...
      { key: 'execution_count', label: '每台设备执行次数', type: 'number', defaultValue: 1 },
    ...
    updateFields: [
      ...
      { key: 'execution_count', label: '每台设备执行次数', type: 'number' },
  ...
  tasks: {
    ...
    createFields: [
      ...
      { key: 'business_platform', label: '业务 App', type: 'select', options: businessPlatformOptions, readonly: true },
      { key: 'runtime_platform', label: '执行平台', type: 'select', options: runtimePlatformOptions, readonly: true },
      {
        key: 'provider',
        label: '供应商',
        type: 'select',
      ...
      { key: 'execution_mode', label: '执行模式', type: 'select', options: executionModeOptions, readonly: true },
      { key: 'execution_count', label: '每台设备执行次数', type: 'number', required: true, defaultValue: 1 },
      { key: 'scheduled_at', label: '计划时间（计划模式必填）', type: 'datetime' },
      { key: 'slot_ids', label: '设备组 / 设备', type: 'slotTree', required: true, defaultValue: [] },
      { key: 'params', label: '覆盖参数', type: 'templateParams', defaultValue: {}, dependencyKey: 'script_key' },
    ],
    updateFields: [],
    inlineActionKeys: ['detail'],
    rowActions: [
      { key: 'detail', label: '任务详情', method: 'GET', icon: 'list', path: (record) => `/api/tasks/${record.id}`, refresh: false },
      { key: 'cancel', label: '取消任务', method: 'POST', icon: 'powerOff', variant: 'danger', path: (record) => `/api/tasks/${record.id}/cancel`, confirm: '确认取消该任务？系统只会取消尚未开始的设备执行记录，已经下发或正在运行的记录不会被干涉。' },
