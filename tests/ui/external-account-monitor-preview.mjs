...
let rows = [{ id: 'fixture-1', business_platform: 'x', profile_url: 'https://x.com/example', remark: '界面测试数据',
  ...
  profile: { display_name: 'External Example', username: 'example', biography: 'Only fixture data, no real collection.', followers_count: 1234, following_count: 56, posts_count: 78 } }]
  ...
  plugins: [{ name: 'isolated-external-monitor-fixture', configureServer(server) {
    server.middlewares.use(async (req, res, next) => {
      ...
      if (req.method === 'POST') {
        ...
        data = { monitor: rows.find(row => row.id === id), total: 1, posts: [{source_key:'post-1',content_url:'https://x.com/example/status/123',report:{text_content:'这是用于验证长正文换行和只读详情的模拟帖子。'.repeat(14),metrics:{like_count:15,comment_count:1},published_at:now,comments:[{author_name:'Fixture Reader',content:'只读采集评论示例',platform_comment_id:'c1'}]}}], snapshots:[{captured_at:now,metrics:{followers_count:1234,collected_post_count:1,collected_like_count:15,collected_comment_count:1}}] }
      } else {
